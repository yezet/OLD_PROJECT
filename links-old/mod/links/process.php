...
elseif ($_GET['act']==='delete_link') {
    ...
    if ($query){
        header("location: ../../main.php?get=manage_link&pesan=3");
    }
	# code...
}

#COPY DAN MOVE OLD DATA
elseif ($_GET['act']=='copy_links') {

	if (isset($_GET['id'])){
        $id = $_GET['id'];
        $query = mysqli_query($koneksi, "INSERT into links SELECT * FROM old_links WHERE url_id='$id'") or die('Error - query: '.mysqli_error($mysqli));
    }
    if ($query){
    	$query_del = mysqli_query($koneksi, "DELETE FROM old_links WHERE url_id='$id'") or die('Error - query: '.mysqli_error($mysqli));
        header("location: ../../main.php?get=link&pesan=1");
    }
	# code...
}
elseif ($_GET['act']==='delete_old_link') {
	if (isset($_GET['id'])){
        $id = $_GET['id'];
        $query = mysqli_query($koneksi, "DELETE FROM old_links WHERE url_id='$id'") or die('Ada kesalahan pada query: '.mysqli_error($mysqli));
    }
    if ($query){
